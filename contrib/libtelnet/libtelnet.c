...
telnet_vsnprintf(char *str, size_t size,
		const char *format, va_list ap)
{
	return (unsigned int) _vsnprintf_s(str, size, _TRUNCATE, format, ap);
}

static INLINE void
telnet_strerror_r(int errnum, char *strerrbuf, size_t buflen)
{
...
telnet_vsnprintf(char *str, size_t size,
	const char *format, va_list ap)
{
	return (unsigned int)vsnprintf(str, size, format, ap);
}
...
#define Q_HIM(q) ((char)(((q).state & 0xF0) >> 4))
#define Q_MAKE(us,him) ((unsigned char)((us) | ((him) << 4)))
...
static telnet_error_t _error(telnet_t *telnet, unsigned line,
	...
	va_start(va, fmt);
	telnet_vsnprintf(buffer, sizeof(buffer), fmt, va);
	va_end(va);
...
static INLINE void _set_rfc1143(telnet_t *telnet, unsigned char telopt,
	...
	if (telnet->q_cnt >= telnet->q_size) {
		...
		if ((qtmp = (telnet_rfc1143_t *)realloc(telnet->q,
			...
			telnet_strerror_r(errno, err_buf, sizeof(err_buf));
			_error(telnet, __LINE__, __func__, TELNET_ENOMEM, 0,
			       "realloc() failed: %s", err_buf);
			return;
		}
...
telnet_t *telnet_init(const telnet_telopt_t *telopts,
		telnet_event_handler_t eh, int flags, void *user_data) {
	/* allocate structure */
...
int telnet_vprintf(telnet_t *telnet, const char *fmt, va_list va) {
	...
	/* format */
	va_copy(va_temp, va);
	rs = telnet_vsnprintf(buffer, sizeof(buffer), fmt, va_temp);
	va_end(va_temp);
	...
	if (rs >= sizeof(buffer)) {
		...
		va_copy(va_temp, va);
		rs = telnet_vsnprintf(output, rs + 1, fmt, va_temp);
		va_end(va_temp);
	}
